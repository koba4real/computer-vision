...
int uniqueNodeIndex(int x, int y, int d, int nx, int ny,int nd) {
    return (y * nx + x) * nd + d;
}
...
void build_graph(Graph<int,int,int>& G,
    ...
    G.add_node(nx*ny*nd);
    // Penalty parameter for disparity continuity
    int penalty = lambda * (1 + (nd - 1) * 4);
    // Loop over all pixels (x, y) and disparity levels (d)
    for (int x = 0; x < nx; x++) {
        for (int y = 0; y < ny; y++) {
            for (int d = 0; d < nd; d++) {
                // Compute the unique node ID for this pixel and disparity level
                int nodeID = uniqueNodeIndex(x, y, d, nx, ny, nd);
                // Calculate pixel coordinates in the first image (I1)
                int xpixel1 = win + zoom * x;
                int ypixel1 = win + zoom * y;
                // Calculate corresponding pixel in the second image (I2) based on disparity d
                int xpixel2 = xpixel1 + (d == 0 ? dmin : (d == nd - 1 ? dmax : dmin + d));
                // Check if the corresponding pixel in I2 is within valid bounds
                bool bound_cond = (xpixel2 + win >= I2.width());
                // Add edges between node and source/sink for terminal weights
                if (d == nd - 1) {
                    // Last disparity level: connect to SINK with weight based on similarity
                    G.add_tweights(nodeID, 0, bound_cond ? INF : (wcc * rho(zncc(I1, I1M, I2, I2M, xpixel1, ypixel1, xpixel2, ypixel1)) + penalty));
                }
                if (d == 0) {
                    // First disparity level: connect to SOURCE with weight based on similarity
                    G.add_tweights(nodeID, bound_cond ? INF : (wcc * rho(zncc(I1, I1M, I2, I2M, xpixel1, ypixel1, xpixel2, ypixel1)) + penalty), 0);
                } else {
                    // Intermediate disparity levels: connect to previous disparity level
                    int nodeID2 = uniqueNodeIndex(x, y, d - 1, nx, ny, nd);
                    G.add_edge(nodeID2, nodeID, bound_cond ? INF : (wcc * rho(zncc(I1, I1M, I2, I2M, xpixel1, ypixel1, xpixel2, ypixel1)) + penalty), 0);
                }
                // Connect current node with neighboring pixels to enforce spatial smoothness
                if (x +1 < nx ) {
                    // Right neighbor
                    int rightNeighborID = uniqueNodeIndex(x + 1, y, d, nx, ny, nd);
                    G.add_edge(nodeID, rightNeighborID, lambda, lambda);
                }
                if (y +1 < ny ) {
                    // Bottom neighbor
                    int bottomNeighborID = uniqueNodeIndex(x, y + 1, d, nx, ny, nd);
                    G.add_edge(nodeID, bottomNeighborID, lambda, lambda);
                }
            }
        }
    }
...
doubleImage decode_graph(Graph<int,int,int>& G, int nx, int ny, int nd, int w2){
    ...
    for (int x = 0; x < nx; x++) {
        for (int y = 0; y < ny; y++) {
            for (int d = 0; d < nd; d++) {
                int xpixel =  win + zoom * x;
                // Calculate the pixel position in the second image for current disparity
                int xpixel_displaced = xpixel + dmin + d;
                // Check if the displaced pixel position is within valid bounds of the second image
                if (xpixel_displaced - win >= 0 && xpixel_displaced + win < w2) {
                    // Get the unique node ID for the (x, y, d) position in the graph
                    int nodeID = uniqueNodeIndex(x, y, d, nx, ny, nd);
                    // If the node is in the SOURCE segment, increment disparity level
                    if (G.what_segment(nodeID) == Graph<int, int, int>::SOURCE) {
                        disparity = dmin + d; // Update to current valid disparity
                    }
                }
            }
            // Store the calculated disparity value in the disparity image
            D(x, y) = disparity;
        }
    }
    return D; // Return the final disparity map
}
